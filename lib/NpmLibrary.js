...
const addTextMarkerToLine = ({ line, textEditor }) => {
...
const calculatePixelsFromLine = (line, { textEditor, view }) => {
...
export default class NpmLibrary {
  constructor(atom, validFiles = ['package.json'], preferences = {}) {
    this.atom = atom;
  ...
  getView(textEditor) {
    return this.atom.views.getView(textEditor);
  }
  ...
  observeMouse(textEditor) {
    const view = this.getView(textEditor);
    const {
  ...
  onMouseMove({ view, textEditor }) {
    ...
    return (event) => {
      ...
      addTooltip({
        dependency,
        ...calculatePixelsFromLine(line, { textEditor, view }),
      ...
      if (marker.isValid() && !marker.isDestroyed()) {
        this.gutters[textEditorKey].decorateMarker(marker, {
          ...
          item: addBadge(
            ...
            {
              dependency,
              ...calculatePixelsFromLine(line, { view, textEditor }),
            },
          ),
        });
      }
