...
const onMouseMove = ({ lineHeight }) => (event) => {
  const {
    x,
    y,
  } = event;

  if (/syntax--quoted/g.test(event.target.className)) {
    console.log('element', event.target);
    const currentLine = Math.floor(y / lineHeight);
    console.log({ x, y, lineHeight });
...
export default class NpmLibrary {
  ...
  observeMouse(textEditor) {
    ...
    const eventListener = onMouseMove({ lineHeight });
