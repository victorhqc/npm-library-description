...
export default class NpmLibrary {
  constructor(validFiles = ['package.json'], preferences = {}) {
    ...
    this.onIsInTooltip = this.onIsInTooltip.bind(this);
    this.updateIsInTooltip = this.updateIsInTooltip.bind(this);

    this.registerHandlers();
  }

  onIsInTooltip(callback) {
    return this.emitter.on('is-in-tooltip', callback);
  }

  updateIsInTooltip(status) {
    console.log('status', status);
    this.isInTooltip = status;
  }

  registerHandlers() {
    this.subscriptions.add(
      this.onIsInTooltip(this.updateIsInTooltip),
    );
  }
  ...
  addMarkerToLine({
    ...
    if (showTooltipWithMouseEvent) {
      return addTextMarkerToLine({ line, textEditor });
    }
  ...
  fetchDependencyInformation(textEditorKey, {
    ...
    return new Promise((resolve, reject) => {
      if (this.isDependencyFetched(textEditorKey, dependency)) {
        const prefetchedDependency = {
          ...this.getDependency(textEditorKey, dependency),
          line,
        };

        this.addDependency(textEditorKey, prefetchedDependency);
        resolve(prefetchedDependency);
        return;
      }
  ...
  destroyEventListener(textEditor) {
    const {
      element,
      eventListener,
    } = this.getEventListener(textEditor);
  ...
  destroy(textEditor) {
    ...
    this.destroyEventListener(textEditor);
  }
}
