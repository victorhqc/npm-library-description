...
export default {
  ...
  config: {
    npmToken: {
      ...
      default: '',
      description: `
Needed to access your private packages. Read more about using tokens here:
...
`,
  ...
  getPreferences() {
    ...
    // Get the npm token.
    const npmToken = atom.config.get(
      'npm-library-description.npmToken',
    );
  ...
  subscribeToConfigurationChange() {
    ...
    this.subscriptions.add(
      atom.config.onDidChange('npm-library-description.npmToken',
        {},
        (event) => {
          const npmToken = event.newValue;
          const preferences = this.getPreferences();
