...
export const cleanDependencyName = name => name.replace(/@|\//g, '');

export const mapDependencies = dependencies => reduce(
  dependencies, (prev, version, dependency) => ([
    ...prev,
    dependency,
  ]), []);
...
export const doesTextMatchJSONKey = text => string => text.match(new RegExp(`"${string}":`));
