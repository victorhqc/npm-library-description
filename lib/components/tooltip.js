...
export const removeTooltip = (element, data) => {
  element.querySelector(`.${TOOLTIP}.${cleanDependencyName(data.name)}`).remove();
};
...
const addAtomLink = (data) => {
  ...
  const dirPath = `${atom.workspace.getActiveTextEditor().buffer.file.getParent().path}/node_modules/${data.name}`;
  const dirObject = new Directory(dirPath);

  if (!dirObject.existsSync()) {
    return undefined;
  }
  ...
  const atomElement = el('a', iconWrapper);
  setAttr(atomElement, {
    className: 'btn',
    title: data.name,
  });
  ...
  atomElement.addEventListener('click', () => {
    atom.project.addPath(dirObject.getPath());
  });
...
export const addTooltip = ({
  ...
  const name = cleanDependencyName(data.name);
  ...
  setAttr(tooltip, {
    className: `${TOOLTIP} ${name} ${positionClass}`,
    style,
  });
